...
for (name in names(data$obsm)) {
  test_that(paste0("Python -> R with obsm and varm '", name, "'"), {
    ad <- anndata::AnnData(
      X = data$X,
      obsm = data$obsm[name],
      varm = data$varm[name]
    ...
    # write to file
    filename <- withr::local_file(paste0("python_to_r_obsmvarm_", name, ".h5ad"))
...
for (name in names(data$obsm)) {
  test_that(paste0("R -> Python with obsm and varm '", name, "'"), {
    # write to file
    filename <- withr::local_file(paste0("r_to_python_obsmvarm_", name, ".h5ad"))
    ad <- HDF5AnnData$new(
      file = filename,
      X = data$X,
      obsm = data$obsm[name],
