...
test_that("Writing a logical matrix preserves its shape and values", {
  ...
  expect_silent(
    write_h5ad_element(value, hdf5_file, "logical_matrix", compression = "none")
  )

  hdf5_file$open_and_defer_close()
  raw <- rhdf5::h5read(hdf5_file$handle, "logical_matrix", native = FALSE)
  # Dense arrays are written transposed on disk (see write_h5ad_dense_array())
  expect_equal(dim(raw), rev(dim(value)))

  roundtrip <- read_h5ad_dense_array_base(hdf5_file, "logical_matrix")
...
test_that("Writing H5AD nullable strings works", {
  ...
  expect_silent(write_h5ad_element(nullable, hdf5_file, "nullable_string"))

  hdf5_file$open_and_defer_close()
  expect_true(hdf5_path_exists(hdf5_file, "/nullable_string"))
  attrs <- rhdf5::h5readAttributes(
    hdf5_file$handle,
    "nullable_string",
    native = FALSE
  ...
  array <- read_h5ad_nullable_string(hdf5_file, "nullable_string")
