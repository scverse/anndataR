...
test_that("AnnDataView combined subsetting through AbstractAnnData", {
  ...
  view <- ad$subset_obs(cell_type == "A")$subset_var(gene_type == "X")
...
test_that("AnnDataView method chaining through AbstractAnnData", {
  ...
  view <- ad$subset_obs(cell_type == "A")$subset_var(
    gene_type == "X"
  )$rename_obs(cell_category = "cell_type")$rename_var(gene_class = "gene_type")
...
test_that("AnnDataView transformation functions through AbstractAnnData", {
  ...
  view3 <- ad$add_transformation("X", function(x) x * 2)$add_transformation(
    "X",
    function(x) x + 1
  )
...
test_that("AnnDataView error handling", {
  ...
  expect_error(
    ad$add_transformation("X", "not_a_function"),
    "transform_func must be a function"
  )

  # Test invalid slot name for transformation
  expect_error(
    ad$add_transformation("invalid_slot", function(x) x),
