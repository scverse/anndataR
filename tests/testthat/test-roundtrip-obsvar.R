...
for (fmt in c("h5ad", "zarr")) {
  ...
  for (name in test_names) {
    ...
    file_r2 <- withr::local_file(
      tempfile(paste0("anndata_r2_", name), fileext = ext)
    )
    ...
    test_that(paste0("reading an AnnData with obs and var '", name, "' (", fmt, ") works"), {
    ...
    test_that(
      paste0("Comparing an anndata with obs and var '", name, "' (", fmt, ") with reticulate works"),
      {
        msg <- message_if_known(
          backend = backend,
          slot = c("obs", "var"),
    ...
    test_that(paste0("Writing an AnnData with obs and var '", name, "' (", fmt, ") works"), {
      msg <- message_if_known(
        backend = backend,
        slot = c("obsp", "varp"),
        dtype = name,
