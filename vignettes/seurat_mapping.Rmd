...
```{r to_Seurat_implicit, eval = FALSE}
...
There must be layer called `counts` or `data` in the final Seurat object. If no AnnData layers are named `counts` or `data`, you must
...
seurat_obj <- to_Seurat(
  ...
  reduction_mapping = list(
    pca = c(key = "PC", obsm = "X_pca", varm = "PCs"),
    umap = c(key = "UMAP", obsm = "X_umap")
  )
)
