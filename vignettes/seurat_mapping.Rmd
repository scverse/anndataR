...
seurat_object@assays$RNA@layers
```
...
metadata is called in the `obs` slot of the anndata object.
...
seurat_obj@assays$RNA@meta.data
```
...
You can provide a mapping for the `graph` slot. The names of the named list refer to what the graph will be called
in the Seurat object. The values refer to what the graph is called in the anndata object. Graphs in an anndata object are
...
There are two ways to correctly specify a mapping for the `misc` slot. 
You must provide a names list.

In the first approach, each element of the named list consists of the name (what it will be called in the Seurat object)
and a value, being a vector with two elements. The first element refers to the name anndata slot and can be `X`, `layers`,
 `obs`, `var`, `obsm`, `varm`, `obsp`, `varp` or `uns`.
the second element points to the specific element in that slot.
...
The second way you can specify a mapping for the `misc` slot is by providing a named list with the name 
of the full anndata slot as a value. This will copy the whole slot to the misc slot in the Seurat object.
...
### varp_mapping
The varp slot is used to store pairwise relations between features. This is not often used.
...
For all the simpler mappings, such as `layers_mapping`, `obs_mapping` and `var_mapping`, you can 
provide a named list with the names of the slots in the AnnData object as names 
and the names of the slots in the Seurat object as values.
...
The mappings fro `obsm`, `varm` and `obsp` all work in the same way: you need to provide a named list,
where the name of the list refers to the name of the slot in the AnnData object.
The values of the list must themselves be vectors of length 2 (for `obsm`) or 2 or 3 (for `varm`, `obsp`, `varp` and `uns`).
The first element of the vector refers to the name of the slot in the Seurat object, the second element refers 
to the name of the slot in the Seurat object. The third element, if provided, refers to the name of the column of that data in that slot.
...
For `uns`, the first element must also be `misc`.
