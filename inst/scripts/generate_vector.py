...
def missing_values_categorical(n, ordered=True):
    ...
    missing_values = pd.Categorical(
        [["Value1", "Value2", "Value3"][i % 3] for i in range(n)],
...
vector_generators = {
    "categorical": lambda n: pd.Categorical(
        [["Value1", "Value2", "Value3"][i % 2] for i in range(n)]
    ),
    "categorical_ordered": lambda n: pd.Categorical(
        [["Value1", "Value2", "Value3"][i % 2] for i in range(n)], ordered=True
    ),
