...
def generate_dataset(n_obs = 10, n_vars = 20, x_type = "numeric_matrix", layer_types = None, obs_types = None, var_types = None, 
    ...
    assert obsm_types is None or all(t in matrix_generators.keys() or t in vector_generators.keys() for t in obsm_types), "Unknown obsm type"
    assert varm_types is None or all(t in matrix_generators.keys() or t in vector_generators.keys() for t in varm_types), "Unknown varm type"
