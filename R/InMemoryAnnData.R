...
InMemoryAnnData <- R6::R6Class(
  ...
  active = list(
    #' @field X `NULL` or an observation x variable matrix (without dimnames)
    #'   consistent with the number of rows of `obs` and `var`.
    X = function(value) {
      if (missing(value)) {
    ...
    #' @field layers `NULL` or a named list with all elements having dimensions
    #'   consistent with `obs` and `var`.
  ...
  public = list(
    ...
    #'   arrays. It must be either `NULL` or a named list, where each element is
    #'   a matrix with `n_vars` rows and an arbitrary number of columns.
