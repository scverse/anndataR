...
to_Seurat <- function(obj) { # nolint
  ...
  seurat <- SeuratObject::CreateSeuratObject(x_assay, meta.data = obs_)
  ...
  for (key in obj$layers_keys()) {
    ...
    seurat[[key]] <- SeuratObject::CreateAssayObject(counts = layer_)
  }
...
#' @rdname Seurat-Conversion
#'
#' @param seurat An object inheriting from Seurat.
...
from_Seurat <- function(seurat, # nolint
  ...
  if (!is.null(assay)) {
    if (!assay %in% names(seurat@assays)) {
      stop(
        ...
        paste0("'", names(seurat@assays), "'", collapse = ", ")
      )
    }
    assay_name <- assay
  } else {
    assay_name <- SeuratObject::DefaultAssay(seurat)

    # If Seurat object contains multiple assays, notify user the Default one is used
    if (length(names(seurat@assays)) > 1) {
      message(
        "There are ", length(names(seurat@assays)), " assays in the Seurat object; using the default assay ('",
  ...
  var_names <- rownames(seurat@assays[[assay_name]])

  # construct var
  # trackstatus: class=Seurat, feature=set_var, status=done
  var <- seurat@assays[[assay_name]]@meta.features
  rownames(var) <- NULL
  ...
  if (!is.null(X)) {
    ...
    if (all(dim(SeuratObject::GetAssayData(seurat, slot = X, assay = assay_name)) == 0)) {
      stop("The '", X, "' slot is empty.")
    }

    assay_data <- SeuratObject::GetAssayData(seurat, slot = X, assay = assay_name)
  ...
  for (slot in slots) {
    if (!all(dim(SeuratObject::GetAssayData(seurat, slot = slot)) == 0)) {
      assay_data <- SeuratObject::GetAssayData(seurat, slot = slot, assay = assay_name)
      dimnames(assay_data) <- list(NULL, NULL)
